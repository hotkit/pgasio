...
namespace pgasio {
    ...
    inline connection<S> handshake(
            S socket, const char *user, const char *database, Y yield) {
        ...
        while (socket.is_open()) {
            ...
            case 'Z':
                return connection<S>(
                        std::move(socket), std::move(settings), process_id,
                        secret);
            default:
                throw std::runtime_error(
                        std::string("Unknown connection message type: ")
                        + header.type);
            }
        }
        throw std::runtime_error(
                "The Postgres connection closed during the initial handshake");
    }
    ...
    template<typename S>
    class connection {
        template<typename Ss, typename Y>
        friend connection<Ss> handshake(Ss, const char *, const char *, Y);
    ...
    inline auto
            unix_domain_socket(boost::asio::io_service &ios, L loc, Y yield) {
