      ...
      public:
        ...
        : m_columns(column_count), m_buffer(bytes) {
            const auto expected_records =
                    (bytes + record_size - 1) / record_size;
        ...
        std::size_t remaining() const { return m_buffer.remaining(); }
        /// The number of bytes that have been used in the block
        std::size_t used_bytes() const { return m_buffer.allocated(); }
            ...
            do {
                ...
                if (next.type == 'D') {
                    ...
                    throw std::logic_error(
                            std::string("Unknown message type: ") + next.type);
                }
        ...
        array_view<const byte_view> fields() const { return m_fields; }
    };
